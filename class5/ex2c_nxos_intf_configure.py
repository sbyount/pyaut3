...
'''
2c. Use Netmiko to push the configurations generated in exercise 2b to the nxos1
device and to the nxos2 device, respectively. Verify you are able to ping between
the devices and also verify that the BGP session reaches the established state.
Note, you might need to use an alternate interface besides Ethernet 2/1 (you can
use either Ethernet 2/1, 2/2, 2/3, or 2/4). Additionally, you might need to use
a different IP network (to avoid conflicts with other students). Your autonomous
system should remain 22, however.

For this exercise you should store your Netmiko connection dictionaries in an
external file named my_devices.py and should import nxos1, and nxos2 from that
external file. Make sure that you use getpass() to enter the password in for
these devices (as opposed to storing the definitions in the file).

Note, this exercise gets a bit complicated when it is all said and done
(templating, pushing configuration to devices, verifying the changes were
successful).
'''

def main():
    # Setup environment for template locations.  Fail if variable not defined
    env = Environment(undefined=StrictUndefined)
    env.loader = FileSystemLoader([".", "./templates/"])

    # Specify jinja2 template file and default interface
    template_file = "nxos_intf_bgp.j2"
    interface = "Ethernet2/4"

    # Variables to populate the template file
    nxos1_vars = {
        "device_name": "nxos1",
        "ibgp_as": "22",
        "interface": interface,
    ...
    # Swap ip and peer addresses for bgp setup
    nxos1_vars["peer_ip"] = nxos2_vars["ipv4_address"]
    nxos2_vars["peer_ip"] = nxos1_vars["ipv4_address"]

    # Add Jinja2 vars dictionary to be included in the Netmiko device dictionary
    ...
    for device in (nxos1, nxos2):
        ...
        # Populate template with J2 vars
        cfg = template.render(**j2_vars)
        # Get device name and print a banner
